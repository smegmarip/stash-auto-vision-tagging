...
2. **`BOOLEAN` settings must default to `false`.** A toggle that has never been touched is indistinguishable in the UI from one that was explicitly set to false, so if the plugin defaulted a boolean to `true`, users would have no way to tell "still on the default" from "I disabled this". Every boolean in the manifest is therefore a **false-by-default opt-out or opt-in flag**. This constraint also forces small-enumeration STRING settings to become binary booleans — any "pick one of N" setting has to be expressed as an opt-in for the non-default value. The manifest is structured accordingly:
    ...
    - **Opt-in flags (user opts into a non-default behavior):** `generateEmbeddings`, `autoTagOnCreate`, `useSemanticsApiAsPrimary`, `useVisionModel`, `useSceneBasedFrameSelection`, `replaceExistingTags`. OFF matches the service / plugin default. ON makes the plugin send the explicit opt-in value (`vision`, `scene_based`, etc.) or switch the internal default (replace policy, semantics-as-primary).
...
**Side-effects scope:** the plugin only writes the `tag_ids` field on the scene. It does **not** populate `details`, captions, markers, or anything else. `scene_summary`, `frame_captions`, `scene_embedding`, and `metadata` from the API response are discarded.
...
**Forwarding `details`:** the plugin reads the Stash scene's `details` field via `findScene` and forwards it on the request as `parameters.details` when non-empty. This is a hint for the classifier — the auto-vision service can also resolve scene details itself by calling Stash with `source_id`, so forwarding is an optimization rather than a requirement. If `scene.details` is empty in Stash, omit the field entirely and let the service do its own lookup.
